...
public class ProjectActivity extends AppCompatActivity implements RecyclerViewItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (bundle != null) {
            ...
            if (action.equalsIgnoreCase(resources.getString(R.string.tec))) {
                ...
                Log.d("bundleid", String.valueOf(projectTypeId));
                getProjectListBasedOnType(projectTypeId,1);
                setupScrolListener();
            }else {
                getProjectList();
            }
    ...
    private void getProjectListBasedOnType(int projectTypeId,int pageNumber) {
        dialog.showDialog();
        Call<ArrayList<userproject>> call = api.fetchProjectListOnuser(userPref.getUserId(),pageNumber);
        call.enqueue(new Callback<ArrayList<userproject>>() {
            @Override
            public void onResponse(@NonNull Call<ArrayList<userproject>> call, @NonNull Response<ArrayList<userproject>> response) {
                dialog.dismissDialog();
                if (response.isSuccessful()) {
                    ArrayList<userproject> stringResponse = response.body();
